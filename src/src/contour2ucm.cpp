...
cv::Point contour_vertex::point() const { return cv::Point(this->x, this->y); }

int contour_edge::size() const { return this->x_coords.size(); }
...
double contour_edge::length() const {
  ...
  double length = sqrt((end.x - start.x) * (end.x - start.x) +
...
void neighbor_compare_2D(const int *pos, const cv::Mat &labels,
                         const cv::Mat &mask, cv::Mat &mask_cmp) {
...
bool is_vertex_2D(const cv::Mat &mask_cmp) {
  ...
  if (nn <= 1)
    ...
    if ((n && (ne || nw)) || (s && (se || sw)) || (e && (ne || se)) ||
        (w && (nw || sw)))
      return true;
    else
      return false;
  } else if ((nn == 3) || (nn == 4)) {
    if ((nw && n && w) || (n && ne && e) || (w && sw && s) || (e && s && se))
      return true;
    else if ((nw && n && ne) || (sw && s && se) || (nw && w && sw) ||
             (ne && e && se))
      return (nn == 3);
      ...
      return true;
    else if ((n && se && sw) || (s && ne && nw) || (w && se && ne) ||
             (e && nw && sw))
      return true;
    else if ((nw && s && e) || (ne && s && w) || (sw && n && e) ||
             (se && n && w))
      return true;
    else
      return false;
...
void super_contour(const cv::Mat &input, cv::Mat &output) {
  ...
  for (size_t i = 0; i < input.rows; i++)
    for (size_t j = 0; j < input.cols; j++) {
      if (j < input.cols - 1)
        H.at<float>(i, j) =
            min(input.at<float>(i, j), input.at<float>(i, j + 1));
      if (i < input.rows - 1)
        V.at<float>(i, j) =
            min(input.at<float>(i, j), input.at<float>(i + 1, j));
    }
  for (size_t i = 0; i < output.rows; i++)
    for (size_t j = 0; j < output.cols; j++) {
      if (i % 2 == 0 && j % 2 == 0)
      ...
      if (j == output.cols)
        output.at<float>(i, j) =
            max(output.at<float>(i, j), output.at<float>(i, j - 1));
      if (i == output.rows)
...
void clean_watersheds(const cv::Mat &input, cv::Mat &output, cv::Mat &labels) {
  ...
  for (size_t i = 0; i < c.rows; i++)
    for (size_t j = 0; j < c.cols; j++) {
      if ((c.at<int>(i, j) == 0) && (output.at<float>(i, j) == 0.0)) {
        ...
        int ind = 0;
        vec[0] =
            max(output.at<float>(i - 2, j - 1), output.at<float>(i - 1, j - 2));
        vec[1] =
            max(output.at<float>(i + 2, j - 1), output.at<float>(i + 1, j - 2));
        vec[2] =
            max(output.at<float>(i + 2, j + 1), output.at<float>(i + 1, j + 2));
  ...
  for (size_t j = 0; j < labels.cols; j++)
    labels.at<int>(labels.rows - 1, j - 1) =
        labels.at<int>(labels.rows - 1, j - 2);
  labels.at<int>(labels.rows - 1, labels.cols - 1) =
      labels.at<int>(labels.rows - 2, labels.cols - 2);
...
void rot90(const cv::Mat &input, cv::Mat &output, int flag) {
  cv::Mat temp;
  input.copyTo(output);
  if (flag == 1)
    output.t();
...
void to_8(const cv::Mat &input, cv::Mat &output) {
  input.copyTo(output);
  for (size_t i = 0; i < output.rows - 1; i++)
    for (size_t j = 0; j < output.cols - 1; j++)
      if (output.at<float>(i, j) > 0 && output.at<float>(i + 1, j + 1) > 0 &&
          output.at<float>(i, j + 1) == 0 && output.at<float>(i + 1, j) == 0)
        output.at<float>(i, j + 1) =
            (output.at<float>(i, j) + output.at<float>(i + 1, j + 1)) / 2.0;
}

int connected_component(const cv::Mat &ws_bw, cv::Mat &labels) {
  cv::Mat mask;
  ...
  for (size_t i = 0; i < n_edges; i++) {
    ...
    for (size_t j = 0; j < _edges[i]->x_coords.size(); j++)
      ws_wt.at<float>(_edges[i]->x_coords[j], _edges[i]->y_coords[j]) =
          max(gPb_ori[orient].at<float>(_edges[i]->x_coords[j],
                                        _edges[i]->y_coords[j]),
              ws_wt.at<float>(_edges[i]->x_coords[j], _edges[i]->y_coords[j]));

    ws_wt.at<float>(v1.x, v1.y) =
        max(gPb_ori[orient].at<float>(v1.x, v1.y), ws_wt.at<float>(v1.x, v1.y));
